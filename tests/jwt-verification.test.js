...
const jwksUrl = process.env.AUTH_JWKS_URL || 'https://authaws-production.up.railway.app/.well-known/jwks.json';
console.log('✅ JWKS URL configured as:', jwksUrl);

// Test 3.1: Test JWKS connectivity
console.log('\n3.1. Testing JWKS Connectivity...');
try {
  // Note: This is a validation script, actual connectivity test would require fetch/http
  if (jwksUrl.includes('authaws-production.up.railway.app')) {
    console.log('✅ JWKS URL points to deployed auth service');
  } else if (jwksUrl.includes('localhost')) {
    console.log('⚠️  JWKS URL points to localhost - make sure auth service is running locally');
  } else {
    console.log('ℹ️  JWKS URL points to custom endpoint:', jwksUrl);
  }
} catch (error) {
  console.log('❌ JWKS connectivity test failed:', error.message);
}
...
const extractedDisplayName = mockPayload.displayName || mockPayload.name;
